...
Only the **sender should close a channel, never the receiver**. Sending on a closed channel will cause a panic. Also, you don't usually need to close channels like files. Closing is only necessary when the receiver must be told that there are no more values coming, so as to **terminate a range loop**.
