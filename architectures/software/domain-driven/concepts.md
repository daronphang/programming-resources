...
Domain logic is the purpose of your modeling. This is where your business rules define the way data gets created, stored, and modified.
...
The ubiquitous language is the channel for all information flowing between developers, domain experts and software. It is critical that developers use the business language in code consciously and as a disciplined rule. Doing so reduces the disconnect between business vocabulary and technological jargon. Characteristics include:

- Industry-standard terminology (HTTP, cache, domain, vendor, etc.) that mirrors the language used in a business context
...
As systems grow too complex to know completely at the level of individual objects, we need techniques for manipulating and comprehending large models. Strategic Design principles provide a guide to design decisions for the model that reduce interdependence of parts and improve clarity and ease of understanding and analysis without reducing their interoperability and synergy.

Strategic Design relates to the big picture: understanding the problems a business must solve; where it achieves competitive advantages; the relationships between teams; and with all this in mind, what software to build. Strategic design decisions help to ensure that the current architecture supports the domain model effectively. These decisions involve identifying core subdomains, defining their relationships, and establishing guidelines for integrating different bounded contexts.
...
Tactical Design refers to patterns, tools and practices that make it simpler to build helpful Domain Models. We use Tactical Design when we have complex business logic to model or when complexity may be introduced in future. Patterns of Tactical Design includes repositories, factories, and services.

In essence, Strategic Design is about determining what we need to build, while Tactical Design is about how we build it.
