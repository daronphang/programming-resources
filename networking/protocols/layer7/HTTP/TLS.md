...
The handshake process requires **two round trips (2RTT)** and is as follows:
...
**One round trip (1RTT)** is required to establish secure connection.
...
For 0-RTT, **data is encrypted** but requests can be replayed. Hence, organizations must still watch out for **targeted replay attacks**, in which attackers intercept original client messages and attempt to send it again later:

1. User transfers $100
2. Attacker records and resends it
3. Server processes it again

As 0-RTT data is **sent and processed** before full handshake verification (if server waited for handshake completion, it would not be 0-RTT and lose performance benefit), the server cannot fully guarantee that the request is fresh and has not been replayed.

0-RTT is only safe for idempotent requests (GET), and can be dangerous for POST requests, e.g. payments, state changes, etc. Servers may mitigate this risk by:

- Allowing 0-RTT for GET
- Reject 0-RTT for POST or sensitive APIs
